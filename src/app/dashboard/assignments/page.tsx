...
import { ASSIGNMENTS } from '../../../consts';

function Item(props: Assignment) {
  ...
  return (
    <>
      ...
      <Card className="w-full p-6 flex shadow-md relative hover:shadow-xl duration-200 transition-all">
        {/* Card Content */}
        <Link href={`/dashboard/assignments/${props.id}`} className="h-full">
    ...
    <div className="w-full container mx-auto">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-3 gap-4">
        {ASSIGNMENTS.map((assignment) => (
          <Item key={assignment.id} {...assignment} />
        ))}
      </div>
    </div>
  );
